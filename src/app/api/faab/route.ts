...
export async function GET(request: NextRequest) {
  try {
    ...
    if (playerId) queryParams.set('player_id', playerId);
    if (budget) queryParams.set('budget', budget);

    // Fetch from Workers API /faab_bands endpoint
    const response = await fetch(`${apiBase}/faab_bands?${queryParams.toString()}`, {
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: `Failed to fetch FAAB bands: ${response.status}` },
    ...
    console.error('Error fetching FAAB bands:', error);

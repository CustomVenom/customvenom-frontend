  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (!res.ok) {
        ...
        console.warn('Yahoo fetch error', { path: '/yahoo/me', status: 'error', request_id: reqId, error: res.error })
...
export function useYahooLeagues() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (!res.ok) {
        const reqId = res.requestId || 'unknown'
        console.warn('Yahoo fetch error', { path: '/yahoo/leagues', status: 'error', request_id: reqId, error: res.error })
        throw new Error(res.error)
      }
